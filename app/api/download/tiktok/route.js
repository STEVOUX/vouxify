...
export async function POST(req) {
  try {
    ...
    const { url, format = 'video', quality = 'best', platform = 'tiktok' } = body;

    if (!url) return NextResponse.json({ error: 'URL is required' }, { status: 400 });

    const jobId = uuid();
    await createJob({ jobId, url, platform, format, quality, status: 'queued' });
    runGenericDownload(jobId, url, format, platform).catch(() => {});
...
async function runGenericDownload(jobId, url, format, platform) {
  const ytDlp = getYtDlpPath();
  const ext = format === 'audio' ? 'mp3' : 'mp4';
  const outputPath = path.join(os.tmpdir(), `vouxify-%(title)s-${jobId}.%(ext)s`);

  try {
    await publishProgress(jobId, 15, `Connecting to ${platform}...`);
    ...
    await execFileAsync(ytDlp, args, { timeout: 120000 });
